...
async function fetchSummaryFromAPI(content) {
  ...
  if (!apiKey) {
    throw new Error(
      "API key not found. Please set your Gemini API key in the extension settings."
    );
  }
  ...
  try {
    ...
    // For gemini-2.5-flash, it's likely still within `candidates[0].content.parts[0].text`.
    if (
      data.candidates &&
